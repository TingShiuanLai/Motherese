...
class ProsodyFeatureExtractor:
    ...
    # def process_reader_files(self, reader, verbose=False):
    #     # print(f"CPU {current_process().name} starting reader {reader}")
    #     reader_samples = []
    #     failed_alignments = 0
    ...
    #     print(f"CPU {current_process().name} finished reader {reader}")
    ...
    #     readers = [
    #         reader for reader in os.listdir(self.lab_root) if reader != ".DS_Store"
    #     ]
    ...
    #         # Use `starmap` because `process_reader_files` takes multiple arguments
    #         results = p.starmap(
    #             self.process_reader_files,
    #             [(reader,) for reader in readers],  # Parameters for each call
    #         )
    #     print("All worker processes have completed")

    #     # `results` is a list of tuples, each containing the result of one call to `process_reader_files`.
    #     # Add the results to the global counters and list of samples.
    #     for reader_samples, fa, tnbsnf in results:
    #         self.samples += reader_samples
    #         failed_alignments += fa
    #         total_nb_syllables_not_found += tnbsnf
    ...
    def process_files(self, lab_root: str = None, wav_root: str = None, verbose=False):
        ...
        # Gather all .lab files
        lab_paths = []
        for root, _, files in os.walk(self.lab_root):
            for f in files:
                if f.endswith(".lab"):
                    lab_paths.append(os.path.join(root, f))

        # Auto-detect folder structure
        flat_structure = all(os.path.dirname(p) == self.lab_root for p in lab_paths)
        shallow_nested_structure = all(len(os.path.relpath(p, self.lab_root).split(os.sep)) == 2 for p in lab_paths)

        if flat_structure:
            print("📁 Detected flat structure (e.g., Sachs-style)")
            structure = "flat"
        elif all(len(os.path.relpath(p, self.lab_root).split(os.sep)) == 2 for p in lab_paths):
            print("📁 Detected shallow nested structure (e.g., Brent-style)")
            structure = "brent"
        else:
            print("📁 Detected nested structure (e.g., LibriSpeech-style)")
            structure = "librispeech"


        # Process each file accordingly
        from tqdm import tqdm
        for lab_path in tqdm(lab_paths, desc="Extracting Features from .lab files"):
            rel_path = os.path.relpath(lab_path, self.lab_root)
            parts = rel_path.split(os.sep)
            filename = os.path.basename(lab_path).replace(".lab", "")

            if structure == "flat":
                reader = "unknown"
                book = "unknown"
                ut_path_lab = lab_path
                ut_path_wav = os.path.join(self.wav_root, f"{filename}.wav")
                ut_path_txt = os.path.join(self.wav_root, f"{filename}.txt")
                ut_path_phoneme_lab = os.path.join(self.phoneme_lab_root, f"{filename}.lab")

            elif structure == "brent":
                folder = parts[0]  # e.g., "c1"
                reader = folder
                book = "unknown"
                ut_path_lab = lab_path
                ut_path_wav = os.path.join(self.wav_root, folder, f"{filename}.wav")
                ut_path_txt = os.path.join(self.wav_root, folder, f"{filename}.txt")
                ut_path_phoneme_lab = os.path.join(self.phoneme_lab_root, folder, f"{filename}.lab")

            else:  # libriSpeech-style
                reader, book, _ = parts
                ut_path_lab = lab_path
                ut_path_wav = os.path.join(self.wav_root, reader, book, f"{filename}.wav")
                ut_path_txt = os.path.join(self.wav_root, reader, book, f"{filename}.txt")
                ut_path_phoneme_lab = os.path.join(self.phoneme_lab_root, reader, book, f"{filename}.lab")

            # print(f"🔍 Looking for .txt at: {ut_path_txt}")
            if not os.path.exists(ut_path_txt):
                if verbose:
                    print(f"⚠ Skipping {filename}: Missing .txt file")
                continue
        ...
        print(f"🟠 Total syllables not found: {total_nb_syllables_not_found}")
        return self.samples
    ...
    #     # Detect flat structure (e.g., Sachs)
    #     lab_paths = []
    #     for root, _, files in os.walk(self.lab_root):
    #         for f in files:
    #             if f.endswith(".lab"):
    ...
    #         # Step 1: collect all (reader, book_name, book_dir, ut) tuples
    #         all_ut_entries = []

    #         for reader in os.listdir(self.lab_root):
    #             if reader == ".DS_Store":
    #                 continue
    #             reader_path = os.path.join(self.lab_root, reader)
    ...
    #                 # CASE 1: flat (book_path is .lab file)
    #                 if book_path.endswith(".lab") and os.path.isfile(book_path):
    #                     all_ut_entries.append((reader, "", reader_path, book))
    #                 # CASE 2: nested (book_path is folder)
    #                 elif os.path.isdir(book_path):
    #                     for ut in os.listdir(book_path):
    #                         if ut.endswith(".lab"):
    #                             all_ut_entries.append((reader, book, book_path, ut))

    #         # Step 2: unified progress bar
    #         for reader, book_name, book_dir, ut in tqdm(all_ut_entries, desc="Extracting Features from .lab files"):
    #             ut_path_lab = os.path.join(book_dir, ut)
    #             ut_path_wav = os.path.join(self.wav_root, reader, book_name, ut.replace(".lab", ".wav"))
    #             ut_path_txt = os.path.join(self.wav_root, reader, book_name, ut.replace(".lab", ".txt"))
    #             ut_path_phoneme_lab = os.path.join(self.phoneme_lab_root, reader, book_name, ut)

    #             if not os.path.exists(ut_path_txt):
    #                 if verbose:
    #                     print(f"⚠ Skipping {ut}: Missing .txt file")
    #                 continue
    ...
    #             except Exception as e:
    #                 if verbose:
    #                     print(f"[❌ Error] Failed to extract features from: {ut} → {e}")
    #                 failed_alignments += 1
    #                 continue
    ...
    #             self.samples.append(sample)
            ...
            #         # ✅ CASE 1: Flat structure (book_path is a .lab file)
            #         if book_path.endswith(".lab") and os.path.isfile(book_path):
            #             ut_files = [book]
            #             book_dir = reader_path  # actual parent dir
            #             book_name = "" # "flat"  # fallback name
            #         # ✅ CASE 2: Nested structure (book_path is a folder)
            #         elif os.path.isdir(book_path):
            #             ut_files = [f for f in os.listdir(book_path) if f.endswith(".lab")]
            #             book_dir = book_path
            #             book_name = book
            #         else:
            #             continue  # Skip anything else
                    
            #         for ut in tqdm(ut_files, desc=f"{reader}/{book_name}", leave=False):
            #         # for ut in ut_files:
            #             if not ut.endswith(".lab"):
            #                 continue
            ...
            # for reader in tqdm(os.listdir(self.lab_root), desc="Extracting Features from Readers"):
            #     if reader == ".DS_Store":
            #         continue
            ...
            #             ut_path_lab = os.path.join(book_path, ut)
            #             ut_path_wav = os.path.join(self.wav_root, reader, book, ut.replace(".lab", ".wav"))
            #             ut_path_txt = os.path.join(self.wav_root, reader, book, ut.replace(".lab", ".txt"))
            #             ut_path_phoneme_lab = os.path.join(self.phoneme_lab_root, reader, book, ut)
